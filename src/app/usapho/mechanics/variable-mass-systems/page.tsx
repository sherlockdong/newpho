...
const NODE_ID = 'USA-05';
const DIFFICULTY_LEVEL = "USAPhO Mechanics physics competition";
const SAMPLE_PROBLEMS = `
### Problem: Accretion and Relativistic Jet Kinetics of a Magnetized Variable-Mass Cylinder

A long, uniform cylindrical column of space debris and gas clouds acts as a continuous mass distribution in space with a linear mass density $\\lambda_0$. A specialized autonomous harvesting spacecraft, modeled as a hollow cylinder of initial mass $M_0$ and radius $R$, is launched directly along the longitudinal axis of this cloud. 

As the spacecraft moves forward at an instantaneous velocity $v(t)$, it continuously sweeps up and absorbs the stationary dust particles at a rate proportional to its velocity, $\\frac{dM}{dt} = \\alpha v(t)$, where $\\alpha$ is a constant capture coefficient. Simultaneously, to prevent clogging, the spacecraft's core processes a fraction of the collected material and continuously ejects it directly backward relative to the spacecraft as a continuous mass distribution (exhaust jet) at a constant relative speed $u$ matching the current ingestion rate.

1. **Set up the differential equation of motion** for this variable-mass system by applying the general conservation of momentum for continuous mass distributions, accounting for both the mass accretion at the front and the continuous exhaust distribution at the rear.
2. **Assuming the spacecraft starts from rest ($v(0) = 0$)** and is subject to a constant external prospective electromagnetic towing force $F_0$, derive an explicit expression for the velocity $v(t)$ as a function of time.
3. **Analyze the asymptotic behavior** of the system: determine the terminal velocity $v_{\\text{term}}$ of the spacecraft as $t \\to \\infty$, and evaluate how the efficiency of this continuous mass transfer depends on the ratio of the exhaust speed $u$ to the initial clearing parameters.
`
...
const SUBTOPIC_NAME = "Variable Mass Systems and Continuous Mass Distribution";
...
export default function MasssPage() {
    const {
      overrideText,
      setOverrideText,
      questionCount,
      ...
      topicName: "USAPhO",
      subtopicName: "Variable Mass Systems and Continuous Mass Distribution",
      difficultyLevel: "USAPhO Mechanics physics competition",
                    ...
                    >
                        <div className={styles.terminalLeft}>
                            <div>
                                ...
                                <p className={styles.terminalSubtitle}>Variable Mass Systems</p>
