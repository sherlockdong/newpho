...
const NODE_ID = 'USA-07';
const DIFFICULTY_LEVEL = "USAPhO Mechanics physics competition";
const SAMPLE_PROBLEMS = `
### Problem: The Precessing Cylinder in a Conical Funnel

A uniform solid cylinder of mass $m$, radius $R$, and height $h = 4R$ is placed inside a fixed, smooth right-circular conical funnel with a semi-vertical angle $\alpha = 45^\circ$. The axis of the funnel is vertically aligned with the gravitational field $\vec{g}$. 

The cylinder is positioned such that its bottom circular edge remains in contact with the inner surface of the cone, and its center of mass $C$ travels in a horizontal circle of radius $d$ ($d > R$) at a constant angular speed $\Omega$. Concurrently, the cylinder spins rapidly about its own longitudinal symmetry axis with a constant spin angular velocity $\omega_s$ ($\omega_s \gg \Omega$).

1. **Determine the components of the inertia tensor** of the cylinder relative to a coordinate system fixed at its center of mass, with the $z$-axis aligned along its longitudinal symmetry axis.
2. **Find the total angular momentum vector $\vec{L}$** of the cylinder relative to its center of mass as a function of $\Omega$, $\omega_s$, and the tilt angle of the cylinder's axis relative to the vertical.
3. **Derive the dynamic equilibrium condition** that relates the required spin velocity $\omega_s$ to the orbital precession frequency $\Omega$ such that the cylinder maintains this steady, non-slipping 3D precessional motion without tipping over or sliding down the funnel.
`
...
const SUBTOPIC_NAME = "Elliptical Orbits and Gravitational Self-Energy";
...
export default function EllipisticalPage() {
    const {
      overrideText,
      setOverrideText,
      questionCount,
      ...
      topicName: "USAPhO",
      subtopicName: "Elliptical Orbits and Gravitational Self-Energy",
                    ...
                    >
                        <div className={styles.terminalLeft}>
                            <div>
                                ...
                                <p className={styles.terminalSubtitle}>Advanced Celestial Mechanics</p>
