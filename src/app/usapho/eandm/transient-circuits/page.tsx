...
const PAST_PROBLEMS = [
    {
        id: "USA_14_P01",
        title: "USAPhO 2018 Problem A2",
        desc: "The problem focuses on transient and steady-state circuit analysis containing a resistor, a capacitor, and a highly non-linear semiconductor device governed by a temperature-dependent current-voltage relationship under sinusoidal driving conditions.",
...
const NODE_ID = 'USA-14';
const DIFFICULTY_LEVEL = "USAPhO Electricity and magnetism physics competition";
const SAMPLE_PROBLEMS = String.raw`plug in Question A2 (from USAPhO 2018)
For this problem, graphical answers should be drawn on the answer sheets graphs
...
I = I_0 * e^(-q*V_0 / (k_B*T)) * (e^(q*V / (k_B*T)) - 1)
where q is the magnitude of the charge on an electron, k_B is Boltzmann’s constant, and T is the
absolute temperature. I_0 and V_0 are non-zero positive constants. Throughout this problem assume
low temperature values k_B*T << q*V_0.
a. On the answer sheets, sketch a graph of the current versus voltage for low temperature values
k_B*T << q*V_0, clearly indicating any asymptotic behavior.
Shown is a schematic for the device. Positive voltage means that the electric potential of the
left hand side of the device is higher than the right hand side. For this device, I_0 = 25 µA and
V_0 = 1.0 V.
VL VR
Below is a circuit made up of these elements. The voltage supplied the circuit is sinusoidal,
V_AB = V_A - V_B = V_s * sin(ωt), and is also shown on answer sheets. The resistance is R = 5.0 Ω and
V_s = 5.0 V.
VB
VA
VC VD
R
b. Sketch the potential difference V_CD = V_C - V_D as a function of time on the answer sheet. For
your convenience, V_AB is shown in light gray. Assume that V_AB has been running for a long
time.
A capacitor is connected to the circuit as shown below. The capacitance is C = 50 mF.
VB
VA
VC VD
C
R
c. Sketch the new potential difference V_CD = V_C - V_D as a function of time on the answer sheet.
For your convenience, V_AB is shown in light gray. Assume that V_AB has been running for a long time.`
...
const SUBTOPIC_NAME = "DC and transient circuits";
...
Note that if the sample problem is graphic, do not make the problems you generate graphic. It will be impossible to answer and evaluate through multiple choice. 
-----------------------
                    ...
                    >
                        <div className={styles.terminalLeft}>
                            <div>
                                ...
                                <p className={styles.terminalSubtitle}>Transient Circuits</p>
