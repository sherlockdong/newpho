...
const PHYSICS_FACTS = [
  "Momentum p=mv changes when you change reference frames, but conservation still holds in every inertial frame, which is a concrete example of Galilean invariance in Newtonian mechanics.",
  "For any isolated system of particles, the total momentum equals total mass times the center-of-mass velocity, p_tot = Mv_cm, even if the internal motion is insanely complicated",
  "A moving object is hard to stop because of its momentum, not its force; stopping it means changing its momentum through an impulse J=FΔt, which directly equals the change in momentum.",
  "A bullet and a slowly rolling bowling ball can have comparable linear momentum, but their vastly different interaction times with a target lead to very different forces and damage profiles.",
  "Total momentum can be zero even in a violent collision if equal and opposite momenta cancel, so “nothing moves overall” despite lots of internal chaos",
  "In an isolated many-particle system, enormous internal forces can act during a collision, but Newton’s third law guarantees their momentum changes cancel pairwise, leaving total momentum unchanged.",
  "Maximum height in projectile motion is reached when vertical velocity becomes zero—not when acceleration becomes zero.",
  "Early physicists saw that something besides momentum was conserved in collisions, leading Leibniz to vis viva (proportional to mv^2), a precursor to kinetic energy, highlighting that momentum and energy conservation encode different symmetry information about collisions.",
  "In the 1920s, apparent violations of energy and momentum conservation in beta decay pushed physicists to postulate the neutrino so that the total 4-momentum still balances, rescuing conservation laws in nuclear processes.",
  "In sports like tennis or baseball, following through with the racket or bat increases the collision time, which increases the impulse and therefore the outgoing ball’s momentum and speed.",
];

export default function MomentumQuizPage() {
  ...
  async function handleGenerateQuiz() {
    ...
    try {
      ...
      const prompt = `
You are an expert physics educator specializing in momentum.
...
`.trim();
  ...
  const parseQuizQuestions = (quizString) => {
    ...
    // Split on ### Question markers (with optional spaces/numbers)
    const rawSections = quizString.split(/### Question \d+/i).filter(Boolean);
  
    // The first split part is usually empty/intro → skip it
    for (let i = 1; i < rawSections.length; i++) {
      ...
      lines.forEach((line) => {
        if (/^[a-d]\)/i.test(line)) {
          ...
          // Everything before options is the question text
...
Return the output as a JSON object with an "analysis" field containing the feedback.`
  ...
  return (
    <div className="quiz-container">
      <h1 className="quiz-title">Systems of Particles and Linear Momentum Quiz</h1>
